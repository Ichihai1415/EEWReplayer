...
namespace EEWReplayer.Utils
{
    public partial class Form_StatisticsMaker : Form
    {
        ...
        private void Form_StatisticsMaker_Load(object sender, EventArgs e)
        {
            ...
            foreach (var kvp in DataConverter.ConvertSource.AreaForecastE_Code2Name)
            {
            ...
            foreach (string filePath in jsonFiles)
            {
                ...
                Form1.f.AddLine(json.Earthquakes[0].OriginTime + " " + json.Earthquakes[0].HypoName + ((warnAreas.Length == 0) ? "" : " " + string.Join(' ', warnAreas!)));
                Form1.f.AddLine("---");
            ...
            foreach (var kvp in codeCounter.OrderBy(x => x.Key))
                Form1.f.AddLine($"{kvp.Key} {DataConverter.ConvertSource.AreaForecastE_Code2Name[kvp.Key]}: {kvp.Value}");
            Form1.f.AddLine("---\nwarn: codes");
